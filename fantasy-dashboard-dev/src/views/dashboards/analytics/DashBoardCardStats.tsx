...
function convertToSimpleWord(str: string) {
  // Check if the string is already in simple word format
  if (str.includes(' ')) {
    return str; // If it already contains a space, return the string as is
  }

  // Convert camelCase to words
  const words = str.replace(/([A-Z])/g, ' $1');
  // Capitalize the first letter and return
  const updatedWords = words.replace(/ ([a-zA-Z])/g, ' $1');
  return updatedWords.charAt(0) + updatedWords.slice(1);
}


const DashboardCardStats = (props: CardStatsHorizontalProps) => {
  // ** Props
      ...
      <CardContent>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography variant='caption' sx={{ textTransform: 'capitalize', fontSize: '1.2rem' }}>{convertToSimpleWord(title.replace('_', " "))}</Typography>
