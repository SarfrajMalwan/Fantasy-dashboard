...
const UserRegister = () => {
  ...
  const registerSchema = yup.object().shape({
    name: yup.string()
      .min(3, 'Name must be 3 character length')
      .required('Name field is required.'),
    email: yup.string().email('Invalid Email address').required('Email is required!'),
    phone: yup.string()
      .min(3, 'Phone must be 10 character length')
      .required('Phone field is required.'),
    password: yup.string().required(),
    confirm_pass: yup.string().label('Confirm password').required().oneOf([yup.ref('password'), null as any], 'Passwords must match'),
