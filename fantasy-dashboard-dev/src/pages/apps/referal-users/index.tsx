...
import useReferalStore, { ReferalUser } from "src/features/referal-users/referal.user";

interface CellType {
  row: ReferalUser
}

let page_title = "Referal Users";



type PlanListColumn = Omit<GridColDef, 'field'> & { field: keyof ReferalUser }
...
const defaultColumns: PlanListColumn[] = [
  ...
  },
  {
    ...
    minWidth: 90,
    field: 'ref_username',
    headerName: 'Referal By',
    editable: false,
    renderCell: ({ row }: CellType) => <Typography variant='body2'>{row?.ref_username}</Typography>
  },
]
