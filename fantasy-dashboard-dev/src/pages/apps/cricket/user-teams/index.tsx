...
import useTeamsStore, { UserTeam } from "src/features/cricket/user.teams.service";

interface CellType {
  row: UserTeam
}

let page_title = "User Teams";



type PlanListColumn = Omit<GridColDef, 'field'> & { field: keyof UserTeam }
...
const Index = ({ read, write, update, del }: GlobalProps) => {
  ...
  const schema = yup.object().shape({
    username: yup.string().label('User Name').meta({ sm: 12 }).required(),
